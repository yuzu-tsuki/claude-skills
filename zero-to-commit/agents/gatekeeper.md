...
Commit-ready checks:
- Gate evidence: for each required preset (dev-debug, ci-gcc11-full, asan-full), do not read the raw log — it can run into the hundreds of KB and the verdict is a few counts. Against `build/<preset>/Testing/Temporary/LastTest.log` run `grep -c '^Test Passed'`, `grep -c '^Test Failed'`, and `grep -ac '^[0-9]*/[0-9]* Testing: '` (tests attempted); confirm zero failed and attempted = passed + failed. Reconcile against `build/<preset>/Testing/Temporary/LastTestsDisabled.log`: disabled tests still appear as vacuous "Test Passed" blocks in the console log, so subtract that count before comparing to the registered test total, or a correct run reads as a false count mismatch. Use `stat` for the log's mtime and confirm it is not older than the newest commit or working-tree change it claims to cover. A missing, stale, or content-empty log is a failure of evidence, not a pass.

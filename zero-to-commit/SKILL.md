...
- **Use every wait — don't idle through it.** While an auditor or the full gates run, draft whatever does not depend on the pending verdict: the tracker section, the reply-file skeleton, commit message bodies. Mark each draft pending-verification and reconcile it when the results land — never publish or commit a draft the round hasn't confirmed.
...
- **Builds are warm.** Never delete, clean, or reconfigure the build directory inside the loop. A cold configure happens at most once per slice, on the first build of a fresh checkout — the review-fix path (`paths.md`) calls that case out.
